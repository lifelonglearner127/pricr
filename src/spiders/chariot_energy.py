...
class ChariotEnergySpider(SpiderBase):
    ...
    def submit_zipcode(self, zipcode: str):
        zipcode_element = self.client.find_element_by_xpath(
            '//div[@class="banner-form animate2 six"]'
            + '//form//input[@class="hpf-zipcode"]'
        )
    ...
    def __get_next_page_button(self) -> WebElement:
        ...
        classes = next_btn.find_element_by_xpath("..").get_attribute(
            "class").split()
    ...
    def analyze_element(self, el: WebElement):
        ...
        price_element = el.find_element_by_xpath('.//p[@class="planprice"]')
        price = re.search(r"(\d+(\.\d+)?)", price_element.text).group()

        plan_element = el.find_element_by_xpath('.//p[@class="planname"]')
